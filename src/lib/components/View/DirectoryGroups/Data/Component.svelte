<script lang="ts">
	...
	const COMPONENT_NAME = 'view-directory-groups-data'
						...
						switch (field[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]) {
							case Domain.Entities.DirectoryGroups.RepositoryName:
								switch (field[MetadataModel.FgProperties.DATABASE_FIELD_COLUMN_NAME]) {
									case Domain.Entities.DirectoryGroups.FieldColumn.ID:
										ntc += 1
										break
									...
									case Domain.Entities.DirectoryGroups.FieldColumn.Description:
										ntc += 1
										break
									case Domain.Entities.DirectoryGroups.FieldColumn.Data:
										ntc += 1
										break
								}
								break
					...
					<section class="flex flex-col gap-y-2">
						...
						>
							...
							<div class="flex gap-x-8">
								...
								{#if addlaunchgroup}
									{@const id = (data[rowIndex] as Domain.Entities.DirectoryGroups.Interface).id}

									{#if Array.isArray(id) && id.length > 0 && id[0]}
										<button
											class="btn btn-sm btn-circle tooltip tooltip-left self-center {themecolor === Domain.Entities.Theme.Color.PRIMARY
												? 'btn-primary tooltip-primary'
												: themecolor === Domain.Entities.Theme.Color.SECONDARY
													? 'btn-secondary tooltip-secondary'
													: 'btn-accent tooltip-accent'}"
											aria-label="launch group {rIndex}"
											onclick={() => {
												goto(
													`${Utils.Env.BasePath}${Domain.Entities.Url.WebsitePaths.Home}?${Domain.Entities.Url.SearchParams.DIRECTORY_GROUP_ID}=${id[0]}`
												)
											}}
											data-tip="launch group"
										>
			...
			<main class="relative z-[1] grid overflow-auto" style="grid-template-columns: subgrid; grid-column: span {gridTemplateColumns};">
				...
				>
					...
					{#if getFieldGroupByFieldColumnName(Domain.Entities.DirectoryGroups.RepositoryName, Domain.Entities.DirectoryGroups.FieldColumn.Description, 0)}
						{@render headerFieldColumn('Description')}
					{/if}
...
{#snippet datumdata(dIndex: number, joinDepth: number = 0)}
	...
	{#if Array.isArray(fieldData)}
		<span>
			{#await getFieldAnyMetadataModel( getFieldGroupByFieldColumnName(Domain.Entities.DirectoryGroups.RepositoryName, Domain.Entities.DirectoryGroups.FieldColumn.Data, joinDepth), [dIndex] ) then fieldAnyMetadataModel}
...
{#snippet datumdescription(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.DirectoryGroups.RepositoryName,
