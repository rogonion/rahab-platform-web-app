<script lang="ts">
	...
	const COMPONENT_NAME = 'view-group-authorization-rules-data'
						...
						switch (field[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]) {
							case Domain.Entities.GroupAuthorizationRules.RepositoryName:
								switch (field[MetadataModel.FgProperties.DATABASE_FIELD_COLUMN_NAME]) {
									case Domain.Entities.GroupAuthorizationRules.FieldColumn.ID:
										ntc += 1
										break
									case Domain.Entities.GroupAuthorizationRules.FieldColumn.RuleGroup:
										ntc += 1
										break
									case Domain.Entities.GroupAuthorizationRules.FieldColumn.Description:
										ntc += 1
										break
								}
						}
							...
							{#await import('../Datum/Component.svelte') then { default: Datum }}
								<main class="z-[1] p-2">
									<Datum
										...
										title={'Authorization Rule Information'}
									></Datum>
								</main>
							{/await}
...
{#snippet datumruleid(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		...
		Domain.Entities.GroupAuthorizationRules.FieldColumn.ID,
		joinDepth
	)}
...
{#snippet datumrulegroup(dIndex: number, joinDepth: number = 0)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.GroupAuthorizationRules.RepositoryName,
		Domain.Entities.GroupAuthorizationRules.FieldColumn.RuleGroup,
		joinDepth
	)}
...
{#snippet listviewdatum(dIndex: number)}
	<div class="flex gap-x-4">
		...
		<span class="self-center">{@render datumgroupruleidandgroup(dIndex)}</span>
	</div>
{/snippet}
