<script lang="ts">
	...
	const COMPONENT_NAME = 'view-group-authorization-rules-search-bar'
	...
	let {
		...
		search = undefined,
		searchtitle = 'Search by rule id, rule section, or description...',
		joindepth = 0
	}: Props = $props()
	...
	let groupAuthorizationRulesQCKey = $derived(
		Utils.MetadataModel.GetGroupQueryPropertiesByDatabaseProperties(metadatamodel, Domain.Entities.GroupAuthorizationRules.RepositoryName, joindepth)
	)
...
<div class="flex h-fit {groupAuthorizationRulesQCKey ? 'w-full' : 'w-fit'} flex-col">
	...
	>
		...
		{#if showquerypanel}
			<button class="join-item btn btn-sm btn-ghost" onclick={showquerypanel} aria-label="Show Query Panel">
