<script lang="ts">
	...
	const COMPONENT_NAME = 'view-iam-credentials-search-bar'
	...
	let {
		...
		search = undefined,
		searchtitle = 'Search by credential name, or email...',
		joindepth = 0
	}: Props = $props()
...
<div class="flex h-fit {iamCredentialsQCKey ? 'w-full' : 'w-fit'} flex-col">
	<section
			...
			>
				<section class="flex flex-1 flex-col gap-y-2">
					...
					{#if directoryGroupsQCKey}
						<label class="input w-full">
							{#if windowWidth > 1000}
								<span class="label">Groups Search</span>
							{/if}
							<input
								class="input-lg w-full"
								type="search"
								placeholder="Search by group name, or group data..."
