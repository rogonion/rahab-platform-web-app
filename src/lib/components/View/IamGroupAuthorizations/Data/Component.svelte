<script lang="ts">
	...
	const COMPONENT_NAME = 'view-iam-group-authorizations-data'
							...
							{#await import('../Datum/Component.svelte') then { default: Datum }}
								<main class="z-[1] p-2">
									<Datum
										...
										title={viewContext === 'iam-credentials' ? 'Group Participant Information' : 'User Role Information'}
										{viewContext}
									></Datum>
								</main>
							{/await}
				...
				>
					...
					{:else}
						{#if getFieldGroupByFieldColumnName(Domain.Entities.GroupRuleAuthorizations.RepositoryName, Domain.Entities.GroupRuleAuthorizations.FieldColumn.GroupAuthorizationRuleID, 1)}
							{@render headerFieldColumn('Rule ID')}
						{/if}
						...
						{#if getFieldGroupByFieldColumnName(Domain.Entities.GroupRuleAuthorizations.RepositoryName, Domain.Entities.GroupRuleAuthorizations.FieldColumn.GroupAuthorizationRuleGroup, 1)}
							{@render headerFieldColumn('Rule Group')}
						{/if}
						...
						{#if getFieldGroupByFieldColumnName(Domain.Entities.IamCredentials.RepositoryName, Domain.Entities.IamCredentials.FieldColumn.OpenidGivenName, 1)}
							{@render headerFieldColumn('Assigneee Given Name')}
						{/if}

						{#if getFieldGroupByFieldColumnName(Domain.Entities.IamCredentials.RepositoryName, Domain.Entities.IamCredentials.FieldColumn.OpenidFamilyName, 1)}
							{@render headerFieldColumn('Assigneee Family  Name')}
						{/if}

						{#if getFieldGroupByFieldColumnName(Domain.Entities.IamCredentials.RepositoryName, Domain.Entities.IamCredentials.FieldColumn.OpenidPreferredUsername, 1)}
							{@render headerFieldColumn('Assigneee Username')}
						{/if}
					{/if}
				</header>
		...
		>
			{#if containerWidth > 375 && noOfRows > tablerowcontentperpage}
				<span class="sticky flex w-fit gap-x-2" style="left: 0px;">
					<span class="input">
						<span class="label">Total No of {viewContext === 'iam-credentials' ? 'Users' : 'User Roles'}:</span>
						<span>{noOfRows}</span>
					</span>
				</span>
			{/if}
...
{#snippet datumruleid(dIndex: number, joinDepth: number = 1)}
...
{#snippet datumrulegroup(dIndex: number, joinDepth: number = 1)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.GroupRuleAuthorizations.RepositoryName,
...
{#snippet datumgroupruleidandgroup(dIndex: number, joinDepth: number = 1)}
...
{#snippet datumdirectorygroupdisplayname(dIndex: number, joinDepth: number = 2)}
	{@const groupName = getDatumFieldData(
		dIndex,
	...
	{#if Array.isArray(groupName) && groupName.length > 0}
		<span class={viewContext === 'iam-credentials' ? 'text-sm italic' : 'text-md'}>
			{groupName[0]}
		</span>
	{:else}
		{@render nodata()}
...
{#snippet datumiamcredentialfamilyname(dIndex: number, joinDepth: number = 1)}
	{@const fieldData = getDatumFieldData(
		dIndex,
...
{#snippet datumiamcredentialgivenname(dIndex: number, joinDepth: number = 1)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.IamCredentials.RepositoryName,
...
{#snippet datumiamcredentialusername(dIndex: number, joinDepth: number = 1)}
	{@const fieldData = getDatumFieldData(
		dIndex,
		Domain.Entities.IamCredentials.RepositoryName,
...
{#snippet datumiamcredentialname(dIndex: number, joinDepth: number = 1)}
	...
	<span class="flex {viewContext === 'iam-credentials' ? 'text-lg font-bold' : 'text-sm italic'}">
	...
	<div class="flex gap-x-4">
		...
		{#if viewContext === 'iam-credentials'}
			<div class="flex flex-col gap-y-1">
				<div class="self-left">{@render datumiamcredentialname(dIndex)}</div>
				<div class="self-left">{@render datumdirectorygroupdisplayname(dIndex)}</div>
			</div>
		{:else}
