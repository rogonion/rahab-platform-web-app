<script lang="ts">
	...
	const COMPONENT_NAME = 'view-iam-group-authorizations-search-bar'
	...
	let {
		...
		search = undefined,
		searchtitle = 'Search by rule id, rule section, or rule description...',
		joindepth = 0
	}: Props = $props()

	let viewQueryCondition = $derived(JSON.parse(JSON.stringify(querycondition)))

	let groupAuthorizationRulesQCKey = $derived(
	...
	let groupAuthorizationRulesFullTextSearchQuery: string = $derived.by(() => {
		if (groupAuthorizationRulesQCKey) {
			if (viewQueryCondition[groupAuthorizationRulesQCKey.qcKey]) {
				return viewQueryCondition[groupAuthorizationRulesQCKey.qcKey][MetadataModel.QcProperties.D_FULL_TEXT_SEARCH_QUERY] || ''
			}
		}
	...
	let iamCredentialsQCKey = $derived(
		Utils.MetadataModel.GetGroupQueryPropertiesByDatabaseProperties(metadatamodel, Domain.Entities.IamCredentials.RepositoryName, joindepth + 1)
	)
	let iamCredentialsFullTextSearchQuery: string = $derived.by(() => {
		if (iamCredentialsQCKey) {
			if (viewQueryCondition[iamCredentialsQCKey.qcKey]) {
				return viewQueryCondition[iamCredentialsQCKey.qcKey][MetadataModel.QcProperties.D_FULL_TEXT_SEARCH_QUERY] || ''
			}
		}
	...
	>
		{#if groupAuthorizationRulesQCKey}
			<input
				...
				value={groupAuthorizationRulesFullTextSearchQuery}
				oninput={(e: Event & { currentTarget: EventTarget & HTMLInputElement }) => {
					if (e.currentTarget.value.length > 0) {
						initQueryCondition(
							groupAuthorizationRulesQCKey.qcKey,
							groupAuthorizationRulesQCKey.fieldGroup[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
							groupAuthorizationRulesQCKey.fieldGroup[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]
						)
						viewQueryCondition[groupAuthorizationRulesQCKey.qcKey][MetadataModel.QcProperties.D_FULL_TEXT_SEARCH_QUERY] = e.currentTarget.value
					} else {
						delete viewQueryCondition[groupAuthorizationRulesQCKey.qcKey]
					}
				...
				<section class="flex flex-1 flex-col gap-y-2">
					{#if iamCredentialsQCKey}
						<label class="input w-full">
							{#if windowWidth > 1000}
								<span class="label">User Credentials Search</span>
							{/if}
							<input
								class="input-lg w-full"
								type="search"
								placeholder="Search by user's name, or email..."
								value={iamCredentialsFullTextSearchQuery}
								oninput={(e: Event & { currentTarget: EventTarget & HTMLInputElement }) => {
									if (e.currentTarget.value.length > 0) {
										initQueryCondition(
											iamCredentialsQCKey.qcKey,
											iamCredentialsQCKey.fieldGroup[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_UID],
											iamCredentialsQCKey.fieldGroup[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME]
										)
										viewQueryCondition[iamCredentialsQCKey.qcKey][MetadataModel.QcProperties.D_FULL_TEXT_SEARCH_QUERY] = e.currentTarget.value
									} else {
										delete viewQueryCondition[iamCredentialsQCKey.qcKey]
									}
									onupdatequerycondition()
								}}
							/>
