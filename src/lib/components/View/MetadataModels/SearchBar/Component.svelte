<script lang="ts">
	...
	const COMPONENT_NAME = 'view-metadata-models-search-bar'
	...
	let {
		...
		search = undefined,
		searchtitle = 'Search by models name, description, tags...',
		joindepth = 0
	}: Props = $props()
	...
	let metadataModelsQCKey = $derived(
		Utils.MetadataModel.GetGroupQueryPropertiesByDatabaseProperties(metadatamodel, Domain.Entities.MetadataModels.RepositoryName, joindepth)
	)
...
<div class="flex h-fit {metadataModelsQCKey ? 'w-full' : 'w-fit'} flex-col">
	<section
