...
export function NewViewSearch(): Domain.Interfaces.MetadataModels.ViewSearch {
	let d: Domain.Interfaces.MetadataModels.ViewSearch = {
		search: new Interfaces.MetadataModels.SearchData(
			`${Domain.Entities.Url.ApiUrlPaths.MetadataModels.Directory.Url}${Domain.Entities.Url.MetadataModelSearchGetMMPath}`,
			`${Domain.Entities.Url.ApiUrlPaths.MetadataModels.Directory.Url}${Domain.Entities.Url.MetadataModelSearchPath}`
		),
		context: 'View Search',
		queryconditions: [],
		...
		async searchdata() {
			...
			State.Loading.value = `Searching ${Domain.Entities.MetadataModelsDirectory.RepositoryName}...`
			try {
				...
				const ERROR = `Search ${Domain.Entities.MetadataModelsDirectory.RepositoryName} failed`
		...
		async getdisplaydata() {
			...
			if (Object.keys(this.search.searchmetadatamodel).length === 0) {
				try {
					...
					const DEFAULT_ERROR = `Get ${Domain.Entities.MetadataModelsDirectory.RepositoryName} metadata-model failed`
			...
			this.search.searchmetadatamodel = MetadataModel.MapFieldGroups(this.search.searchmetadatamodel, (property: any) => {
				if (
					...
					property[MetadataModel.FgProperties.DATABASE_TABLE_COLLECTION_NAME] === Domain.Entities.MetadataModelsDirectory.RepositoryName &&
					property[MetadataModel.FgProperties.DATABASE_FIELD_COLUMN_NAME] === Domain.Entities.MetadataModelsDirectory.FieldColumn.LastUpdatedOn
				) {
